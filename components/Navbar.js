...
export default function Navbar() {
  ...
  const userInitials = `${userLabel}`.split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]?.toUpperCase() || '').join('')
          ...
          {!isAdminRoute &&
            (user ? (
              <div ref={profileRef} className="relative hidden sm:block">
                ...
                >
                  <span className="inline-flex h-6 w-6 items-center justify-center rounded-full border border-white/10 bg-white/10 text-[11px] font-semibold text-white">
                    {userInitials || 'N'}
                  </span>
                  <span>{userLabel}</span>
                </button>
                ...
                {profileOpen && (
                  <div className="absolute right-0 top-[calc(100%+12px)] z-50 w-[320px] rounded-[1.75rem] border border-white/10 bg-[rgba(12,18,32,0.96)] p-4 text-white shadow-[0_24px_70px_rgba(2,6,23,0.45)] backdrop-blur-xl">
                    <div className="rounded-[1.5rem] border border-white/10 bg-white/5 p-5 text-center">
                      <div className="mx-auto inline-flex h-20 w-20 items-center justify-center rounded-full border border-white/10 bg-slate-950 text-2xl font-semibold">
                        {userInitials || 'N'}
                      </div>
