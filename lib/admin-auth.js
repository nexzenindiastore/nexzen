...
function verifyPassword(password, storedPassword) {
  ...
  if (storedBuffer.length !== candidateHash.length) return false
...
export async function authenticateAdmin(username, password) {
  ...
  if (isLegacyPlaintextPassword(admin.password)) {
    ...
    await prisma.$executeRaw`
      ...
      WHERE "id" = ${admin.id}
    `
    return { ...admin, password: upgradedPassword }
  }
...
export async function createAdminSession(adminId, context = {}) {
  ...
  return {
    token,
    session: { id: sessionId, adminId, expiresAt },
  }
}
...
export async function getAdminSession(token) {
  ...
  return {
    ...session,
    admin: { id: session.adminId, username: session.adminUsername },
  }
}
...
export async function deleteAdminSession(token) {
  ...
  await prisma.$executeRaw`
    DELETE FROM "admin_sessions" WHERE "tokenHash" = ${hashSessionToken(token)}
  `
}